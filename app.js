...
if (!process.env.DEBUG)
    console.log = () => { Log.debug.apply(Log, arguments) };
...
async.series([
    ...
    (next) => {
        if (global.options.mysql.length == 0) return next();
        // example config:
        // {
        //     "name": "foo",
        //     "host": "xxx.com",
        ...
        MySqlClient.load(global.options.mysql, next);
    }
], (err) => {
    if (err) {
        ...
        app.use('/', routes);
        app.use('/1/emotes', emotes);
        app.use('/1/polls', polls);
